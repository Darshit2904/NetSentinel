...
def get_tls_handshake_details(url, user_agent):
    host = url.split("://")[-1]
    port = 443
...
# Testing the TLS handshake with different user agents
results = []
for user_agent in user_agents:
    result = get_tls_handshake_details('https://www.google.com', user_agent)
    results.append(result)

# Print the results in the desired format
print("TLS Handshake Simulation Results:\n")
for result in results:
    print(f"User-Agent: {result['user_agent']}")
    if result.get('curve') != "N/A":
        print(f"Curve: {result['curve']}")
    if result.get('protocol') != "N/A":
        print(f"Protocol: {result['protocol']}")
    if result.get('cipher_suite') != "N/A":
        print(f"Cipher Suite: {result['cipher_suite']}")
    if result.get('protocol_code') != "Unknown Protocol Code":
        print(f"Protocol Code: {result['protocol_code']}")
    if result.get('cipher_suite_code') != "Unknown Cipher Suite Code":
        print(f"Cipher Suite Code: {result['cipher_suite_code']}")
    if result.get('curve_code') != "Unknown Curve Code":
        print(f"Curve Code: {result['curve_code']}")
    print("\n")
